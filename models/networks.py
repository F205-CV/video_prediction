...
# Content Encoder
#####################
def content_E(input_nc, output_nc, ngf, which_model_E, norm='batch', use_dropout=False,gpu_ids=[]):
    netCE = None
    ...
    if which_model_E == 'resnet_9blocks':
        netCE = ResnetEncoder(input_nc, output_nc, ngf, norm_layer = norm_layer, use_dropout = use_dropout, n_blocks=9, gpu_ids=gpu_ids)
    elif which_model_E == 'resnet_6blocks':
        netCE = ResnetEncoder(input_nc, output_nc, ngf, norm_layer=norm_layer,use_dropout=use_dropout, n_blocks=6, gpu_ids=gpu_ids)
    elif which_model_E == 'resnet_3blocks':
        netCE = ResnetEncoder(input_nc, output_nc, ngf, norm_layer=norm_layer,use_dropout=use_dropout, n_blocks=3, gpu_ids=gpu_ids)
    #elif which_model_E == 'unet_128':
    #    netCE = UnetGenerator(input_nc, output_nc, 7, ngf, norm_layer=norm_layer,use_dropout=use_dropout, gpu_ids=gpu_ids)
    #elif which_model_E == 'unet_256':
    #    netCE = UnetGenerator(input_nc, output_nc, 8, ngf, norm_layer=norm_layer,use_dropout=use_dropout, gpu_ids=gpu_ids)
    else:
...
# Motion Encoder
#####################
def motion_E(input_nc, output_nc, ngf, which_model_E, norm='batch', use_dropout=False,gpu_ids=[]):
    netME = None
    use_gpu = len(gpu_ids) > 0
    norm_layer = get_norm_layer(norm_type = norm)
    if use_gpu:
        assert(torch.cuda.is_available())
    if which_model_E == 'resnet_9blocks':
        netME = ResnetEncoder(input_nc, output_nc, ngf, norm_layer=norm_layer,use_dropout=use_dropout,n_blocks=9,gpu_ids=gpu_ids)
    elif which_model_E == 'resnet_6blocks':
        netME = ResnetEncoder(input_nc, output_nc, ngf, norm_layer=norm_layer,use_dropout=use_dropout,n_blocks=6,gpu_ids=gpu_ids)
    elif which_model_E == 'resnet_3blocks':
        netME = ResnetEncoder(input_nc, output_nc, ngf, norm_layer=norm_layer,use_dropout=use_dropout,n_blocks=3,gpu_ids=gpu_ids)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % which_model_E)
    if len(gpu_ids) > 0:
...
# Motion Predictor
#####################
def motion_P(shape,hist_len,input_nc, output_nc, ngf, norm='batch', use_dropout=False,gpu_ids=[]):
    netMP = None
    use_gpu = len(gpu_ids) > 0
    norm_layer = get_norm_layer(norm_type = norm)
    if use_gpu:
...
# Overall Generator
#####################
def define_G(shape,hist_len,input_nc, output_nc, ngf, norm='batch', use_dropout=False,gpu_ids=[]):
    netG = None
    use_gpu = len(gpu_ids) > 0
    norm_layer = get_norm_layer(norm_type = norm)
    if use_gpu:
